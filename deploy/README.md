...
巧侬专用 CD 的完整信任边界、部署流程和统一安装清单见 `docs/CD方案与接入说明.md`。`qiaonong-cd-dispatch`、`qiaonong-cd-entrypoint` 和 `qiaonong-cd-backup.py` 是服务器固定脚本的仓库来源，不得由普通代码部署直接覆盖 `/usr/local/sbin/` 中的 root-owned 副本。当前阶段不修改服务器 SSH、sudoers、虚拟环境权限或共享构建锁配置。
...
仓库中的 CD 文件是统一维护窗口的待安装来源。目前服务器仍使用既有人工部署方式；普通业务部署不会覆盖 `/usr/local/sbin` 固定副本。此版本通过 CI 后，`.github/workflows/`、`deploy/`、CD 方案文档及服务器权限设计进入冻结状态，除非用户明确要求或发现阻断部署的缺陷。
