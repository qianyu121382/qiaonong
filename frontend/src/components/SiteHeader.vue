...
<template>
  <header class="site-header">
    ...
    <nav :class="{ open: menuOpen }" @click="closeMenu">
      ...
      <div v-for="category in siteState.categories" :key="category.id" class="nav-group">
        <router-link :to="`/products/${category.slug}`">{{ category.name }}</router-link>
        <div v-if="category.children?.length" class="nav-dropdown">
          <router-link v-for="child in category.children" :key="child.id" :to="`/products/${child.slug}`">{{ child.name }}</router-link>
        </div>
      </div>
